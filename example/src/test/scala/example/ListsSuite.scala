 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("max of list with duplicates") {
    assert(max(List(3,4,2,4,1)) === 4)
  }

  test("max of list with negative number") {
    assert(max(List(3,4,-2,1)) === 4)
  }

  test("max of list with zeros") {
    assert(max(List(-2,0,1)) === 1)
  }

  test("sum of list with duplicates") {
    assert(sum(List(3,4,2,4,1)) === 14)
  }

  test("sum of list with negative number") {
    assert(sum(List(3,4,-2,1)) === 6)
  }
